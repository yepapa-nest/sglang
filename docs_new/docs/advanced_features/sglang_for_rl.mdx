...
- The scheduler keeps one weight-version id per KV slot and stamps every slot each forward writes. When a request's prefill finishes its prompt slots are read back and run-length encoded, so a span says which weights computed the KV that prompt token is served from.
...
- A retracted request is re-prefilled and re-recorded, so it reports whatever its new KV came from, not the original prefill.
- Aborted requests carry the field once their prefill has finished; a request aborted mid-prefill has none. OpenAI-compatible non-streaming responses surface it under `metadata`, and streaming responses only on the final chunk.
